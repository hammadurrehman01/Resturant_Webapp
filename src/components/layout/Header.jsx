      ...
      <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-3.5">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <div className="relative grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 font-bold text-white text-lg shadow-md shadow-brand-500/20 transition-transform duration-200 group-hover:scale-105">
            {(restaurant?.name?.[0] || 'K').toUpperCase()}
        ...
        <nav className="ml-auto hidden items-center gap-2 sm:flex">
          {NAV.map((n) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `px-3.5 py-2 text-xs font-black tracking-widest transition-all duration-200 ${
                  isActive
          ...
          <nav className="mt-2 flex flex-col gap-0.5">
            {NAV.map((n) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `rounded-xl px-4 py-3 text-xs font-black tracking-widest transition ${
                    isActive ? 'bg-stone-900 text-gold-400' : 'text-stone-300 hover:bg-stone-900'
                  }`
                }
              >
                {n.label}
